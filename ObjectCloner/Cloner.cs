...
namespace ObjectCloner
{
    ...
    public class Cloner
    {
        public static T Clone<T>(T instance)
        {
            ...
            var dyn_inst = Activator.CreateInstance(type, instance);
            return (T)dyn_inst;
        }
    }
    ...
    internal class DynamicConstructorAssembler
    {
        ...
        private static void EmitListCopy(ILGenerator il, PropertyInfo source_property)
        {
            ...
            {
                b1 = DefineConstructor(item_type);
            }
        ...
        internal static DynamicConstructorInfo DefineConstructor(Type baseType)
        {
            ...
            //ConstructorBuilder cb = dci.Builder.DefineConstructor(MethodAttributes.Public, CallingConventions.Standard, new Type[] { baseType });
            // ConstructorDefines[baseType] = cb;
            return dci;
        }



        //private static Dictionary<Type, ConstructorBuilder> ConstructorDefines;
    ...
    public class _DynamicConstructorAssembler
    {

        public static DynamicType CreateConstructor(Type baseType)
        {
            DynamicType dtype = null;
            if (DynamicTypes.TryGetValue(baseType, out dtype))
            {
                return dtype;
            }
            ...
            string typeName = baseType.Name + "_Dynamic";
            TypeBuilder tb = mb.DefineType(typeName, TypeAttributes.Public, baseType);

            // Criando um novo construtor que recebe um objeto do mesmo tipo
            ConstructorBuilder cb = tb.DefineConstructor(MethodAttributes.Public, CallingConventions.Standard, new Type[] { baseType });
            ILGenerator il = cb.GetILGenerator();


            var dyn = new DynamicType() { Original = baseType, Prototype = tb.CreateTypeInfo() };
            DynamicTypes.Add(baseType, dyn);
        ...
        private static void EmitObjectAssignor(ILGenerator il, PropertyInfo source_property)
        {
            var type     = source_property.PropertyType;
            var dyn_type = CreateConstructor(type);
            var temp     = il.DeclareLocal(typeof(bool));

            // Carrega "obj" (argumento 1) e chama get_Atributo
            il.Emit(OpCodes.Ldarg_1);
            MethodInfo getAtributo = source_property.GetGetMethod();
            il.Emit(OpCodes.Callvirt, getAtributo);

            // Carrega null e compara: se (obj.Atributo != null)
            il.Emit(OpCodes.Ldnull);
            il.Emit(OpCodes.Cgt_Un); // Se o valor for diferente de null, empurra 1 (true); senão, 0 (false)
            il.Emit(OpCodes.Stloc, temp);

            // Verifica a condição: se o valor for false (0), pula o bloco
            Label skipLabel = il.DefineLabel();
            il.Emit(OpCodes.Ldloc, temp);
            il.Emit(OpCodes.Brfalse_S, skipLabel);

            // Corpo do if:
            // Carrega "this" (argumento 0)
            il.Emit(OpCodes.Ldarg_0);
            // Carrega novamente "obj" (argumento 1) e chama get_Atributo para obter o valor
            il.Emit(OpCodes.Ldarg_1);
            il.Emit(OpCodes.Callvirt, getAtributo);

            // Cria nova instância de ObjetoTesteFilho chamando o construtor que recebe ObjetoTesteFilho
            ConstructorInfo ctorFilho = dyn_type.Prototype.GetConstructor(new Type[] { type });
            il.Emit(OpCodes.Newobj, ctorFilho);

            // Chama o setter set_Atributo para atribuir o novo objeto à propriedade
            MethodInfo setAtributo = source_property.GetSetMethod();
            il.Emit(OpCodes.Callvirt, setAtributo);
        ...
        private static void EmitArrayCopy(ILGenerator il, PropertyInfo source_property)
        {
            ...
            // Obtém (ou cria) o tipo dinâmico para o construtor de cópia do item.
            var dynType      = CreateConstructor(itemType);
            var itemCopyCtor = dynType.Prototype.GetConstructor(new Type[] { itemType });
            ...
            // Cria uma nova instância: new ObjetoTesteFilho(obj.Array[ix])
            il.Emit(OpCodes.Newobj, itemCopyCtor);
            // Armazena o resultado em newArray[ix]
            il.Emit(OpCodes.Stelem_Ref);
        ...
        private static void EmitListCopy(ILGenerator il, PropertyInfo source_property)
        {
            ...
            var item_type = list_type.GetGenericArguments()[0];
            var dyn_type  = CreateConstructor(item_type);
            var item_ctor = dyn_type.Prototype.GetConstructor(new Type[] { item_type });
            var get_child = source_property.GetGetMethod();
            var set_child = source_property.GetSetMethod();
            ...
            il.EmitCall(OpCodes.Call, get_child, null);                    // Chama get_Children para acessar a lista
            il.Emit(OpCodes.Ldloc, localChild);                            // Carrega a variável a
            il.Emit(OpCodes.Newobj, item_ctor);                            // Cria new ObjetoTesteFilho(a)
            il.EmitCall(OpCodes.Callvirt, list_add, null);                 // Chama Add no List<ObjetoTesteFilho>
        ...
        private static Dictionary<Type, DynamicType> DynamicTypes = new Dictionary<Type, DynamicType>();

        static _DynamicConstructorAssembler()
        {
            DynamicTypes       = new Dictionary<Type, DynamicType>();
            AssemblyName aName = new AssemblyName("DynamicConstructorAssembler");
